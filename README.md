<h1 align="center">
  <a href="https://github.com/omikhan4901">
    <img src="https://readme-typing-svg.demolab.com?font=Fira+Code&weight=700&size=22&pause=1000&color=2DD4BF&center=true&vCenter=true&width=900&lines=Hey+%F0%9F%91%8B+%E2%80%94+I%27m+Omi;Intern+SWE+%40+Automata+One+%C2%B7+one.bd;Final-year+CSE+%40+North+South+University;AI+%2F+ML+Inference+%C2%B7+Full-stack+Engineer;Privacy-first+mobile+apps+%C2%B7+Building+in+public" alt="Typing SVG" />
  </a>
</h1>

<p align="center">
  <a href="https://github.com/omikhan4901">
    <img src="https://komarev.com/ghpvc/?username=omikhan4901&label=Profile+views&color=2dd4bf&style=for-the-badge" />
  </a>
  <a href="https://github.com/omikhan4901?tab=followers">
    <img src="https://img.shields.io/github/followers/omikhan4901?label=Followers&style=for-the-badge&color=2dd4bf" />
  </a>
  <a href="https://one.bd">
    <img src="https://img.shields.io/badge/Automata%20One-Intern%20SWE-0d9488?style=for-the-badge&logoColor=white" />
  </a>
  <a href="https://linkedin.com/in/YOUR_LINKEDIN">
    <img src="https://img.shields.io/badge/LinkedIn-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white" />
  </a>
  <a href="https://cse299-omi.vercel.app">
    <img src="https://img.shields.io/badge/ResumeX-Live%20Demo-2dd4bf?style=for-the-badge&logo=vercel&logoColor=white" />
  </a>
</p>

---

## 🧑‍💻 About Me

```bash
omi@automata-one:~$ whoami
# Mehboob Ehsan Khan Omi — Intern SWE @ Automata One · one.bd · NSU final year

omi@automata-one:~$ cat about.sh

  NAME="Mehboob Ehsan Khan Omi"
  LOCATION="Dhaka, Bangladesh"
  EDUCATION="B.Sc in CSE @ North South University"  # final semester
  ROLE="Intern Software Engineer @ Automata One (one.bd)"

  WHAT_I_SHIP=(
    "Deep learning inference pipeline: N-best rescoring + MBR on distributed GPUs"
    "BondhuKoi: privacy-first campus location app — zero raw GPS stored"
    "ResumeX: AI resume builder with Gemini — live & deployed"
  )

  STACK=("Next.js" "React Native" "FastAPI" "Node.js" "Express" "Discord.js" "Postgres" "PostGIS")
  BACKGROUND="Java + JS web dev roots. Got pulled into AI/ML. Now I build both ends."
  CURRENTLY_INTO=("AI inference systems" "LLM reasoning" "full-stack engineering" "system design")
  LEARNING_NEXT=("DSA + competitive programming (just getting started!)" "advanced TypeScript")

  # fun facts
  echo "→ Shipped two mobile apps while studying for finals"
  echo "→ Built Discord bots before it was cool (Discord.js v12 era)"
  echo "→ Best debugger is a good night's sleep"

omi@automata-one:~$ _
```

---

## 🔬 Featured Projects

<table>
  <tr>
    <td width="50%" valign="top">
      <h3>🤖 <a href="https://github.com/omikhan4901/asr">DL Inference Pipeline</a></h3>
      <p><em>CSE499 Capstone &nbsp;·&nbsp; Whisper &nbsp;·&nbsp; Python &nbsp;·&nbsp; Modal GPUs</em></p>
      <p>Inference-only deep learning system: N-best hypothesis generation + Minimum Bayes Risk decoding, no model fine-tuning required.</p>
      <ul>
        <li>Distributed GPU inference via Modal — <strong>WER drop on 2,271 test utterances</strong></li>
        <li>VAD chunking → group-aware batching → probability-weighted MBR</li>
        <li>Recovers ~42% of oracle headroom at inference time</li>
      </ul>
      <p><a href="https://asr-phi.vercel.app">🌐 Live speech-to-SRT demo</a></p>
    </td>
    <td width="50%" valign="top">
      <h3>📍 <a href="https://github.com/omikhan4901/bondhukoi">BondhuKoi</a></h3>
      <p><em>React Native (Expo) &nbsp;·&nbsp; Fastify &nbsp;·&nbsp; Supabase &nbsp;·&nbsp; PostGIS</em></p>
      <p>Campus location sharing where friends see zones — not coordinates.</p>
      <ul>
        <li>Server-side point-in-polygon on PostGIS geofences</li>
        <li><strong>Zero raw GPS ever stored</strong> — privacy by design</li>
        <li>Sleep window, kill-switch, JWT + Google sign-in</li>
      </ul>
      <p><a href="https://youtube.com/shorts/_Thc_gN7YzY">▶️ Demo video</a></p>
    </td>
  </tr>
  <tr>
    <td width="50%" valign="top">
      <h3>📄 <a href="https://github.com/omikhan4901/cse299">ResumeX</a></h3>
      <p><em>MERN &nbsp;·&nbsp; Gemini &nbsp;·&nbsp; Vite &nbsp;·&nbsp; Ant Design</em></p>
      <p>Upload your old resume, get it rewritten by AI and exported ATS-ready.</p>
      <ul>
        <li>PDF/DOCX parse → bullet rewriting via Gemini</li>
        <li>Resume chatbot + 9 ATS-friendly templates</li>
        <li>Deployed and live</li>
      </ul>
      <p><a href="https://cse299-omi.vercel.app">🌐 Try it live</a></p>
    </td>
    <td width="50%" valign="top">
      <h3>🏢 <a href="https://github.com/omikhan4901/327_Company_Management_System">Company Management System</a></h3>
      <p><em>FastAPI &nbsp;·&nbsp; SQLite &nbsp;·&nbsp; pytest</em></p>
      <p>Full HR & payroll system with real software engineering discipline.</p>
      <ul>
        <li>RBAC, nested departments, payroll strategies, PDF reports</li>
        <li>Explicit design patterns: Adapter, Composite, Strategy, Observer</li>
        <li>Full pytest suite: auth, payroll, attendance, tasks</li>
      </ul>
    </td>
  </tr>
</table>

<sub>Also on GitHub: an xv6 MLFQ scheduler in C (3-level queue, priority boost, aging), and a ride-share app with Socket.io + Google Cloud Vision OCR on student IDs.</sub>

---

## 🛠️ Tech Stack

<p align="center">
  <img src="https://skillicons.dev/icons?i=java,js,ts,python,cpp,html,css&theme=dark" />
</p>
<p align="center">
  <img src="https://skillicons.dev/icons?i=nextjs,react,fastapi,nodejs,express,discord&theme=dark" />
</p>
<p align="center">
  <img src="https://skillicons.dev/icons?i=mongodb,postgres,supabase,git,github,vscode,linux,bash&theme=dark" />
</p>

---

## 📊 GitHub Stats

<div align="center">
  <img src="https://github-readme-streak-stats.herokuapp.com/?user=omikhan4901&theme=merko&hide_border=true&background=0d1117&ring=2dd4bf&fire=2dd4bf&currStreakLabel=2dd4bf&sideLabels=2dd4bf&dates=a8b2d8" />
</div>

---

## 🎮 Contribution Activity

<div align="center">
  <img src="https://ghchart.rshah.org/omikhan4901" alt="omikhan4901's contribution graph" width="100%" />
</div>

---

## 📈 Profile Breakdown

<div align="center">
  <img src="https://github-profile-summary-cards.vercel.app/api/cards/profile-details?username=omikhan4901&theme=github_dark&title_color=2dd4bf&chart_color=2dd4bf&animation=draw" width="100%" />
</div>

<div align="center">
  <img src="https://github-profile-summary-cards.vercel.app/api/cards/repos-per-language?username=omikhan4901&theme=github_dark&title_color=2dd4bf&chart_color=2dd4bf&animation=draw" width="32%" />&nbsp;
  <img src="https://github-profile-summary-cards.vercel.app/api/cards/most-commit-language?username=omikhan4901&theme=github_dark&title_color=2dd4bf&chart_color=2dd4bf&animation=draw" width="32%" />&nbsp;
  <img src="https://github-profile-summary-cards.vercel.app/api/cards/stats?username=omikhan4901&theme=github_dark&title_color=2dd4bf&animation=draw" width="32%" />
</div>

---

## 💼 Experience & Education

<div align="center">

| | Role | Where | Period |
|:---:|:---|:---|:---|
| 💼 | **Intern Software Engineer** | [Automata One](https://one.bd) | 2026 – Present |
| 🎓 | **B.Sc in CSE** | North South University | Graduating 2026 |

</div>

---

<div align="center">
  <i>⚡ "Build clean. Ship fast. Learn everything." ⚡</i>
  <br/><br/>
  <b>Building things that don't exist yet 🌱</b>
</div>